...
#set the path
path="clustering/hamming distanceNo0 on ada/"


#Read the template file => Assign 1~39 for "part" variable and output 39 .R files
conn=file(paste(path,"/hamming_No0_template.R",sep=""))
txt <-readLines(conn)
close(conn)
...
for(i in 1:39){
  ...
  conn<-file(paste(path,"hamming_No0_part_",i,".R",sep=""))
...
conn=file(paste(path,"/hamming_No0_template.lsf",sep=""))
txt <-readLines(conn)
close(conn)
#How to solve the warning "incomplete final line found on '......'":
#https://stackoverflow.com/questions/5990654/incomplete-final-line-warning-when-trying-to-read-a-csv-file-into-r
...
for(i in 1:39){
  txt.out=sub("Rscript",paste("Rscript hamming_No0_part_",i,".R",sep=""),txt) #assign 1~39 for "part" variable
  conn<-file(paste(path,"hamming_No0_part_",i,".lsf",sep=""))
  writeLines(txt.out, conn)
  close(conn)
}
...
txt.out=c()
for(i in 1:39){
  txt.out=c(txt.out, paste("bsub < hamming_No0_part_",i,".lsf",sep="")) ##\n is not needed here. (But I don't know why)
  
}

conn<-file(paste(path,"hamming_No0_allJobs.txt"))
writeLines(txt.out, conn)
close(conn)
