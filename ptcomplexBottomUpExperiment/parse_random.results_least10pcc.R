...
pwyBottomUpExp.random.least10pcc=data.frame()
for(i in j){
  file.name=paste("avg.random_tables.j",i,".RData",sep="")
  path=paste("pwyBottomToTopExperiment/random.result.FromADA_least10pcc/",file.name,sep="")
  load(path)
  pwyBottomUpExp.random.least10pcc=rbind(pwyBottomUpExp.random.least10pcc, avg.random_tables)
}

save(pwyBottomUpExp.random.least10pcc,file="Data/pwyBottomUpExp.random.least10pcc.RData")
