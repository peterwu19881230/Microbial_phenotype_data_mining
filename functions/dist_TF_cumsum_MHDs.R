...
dist_TF_cumsum_MHDs=function(data,distance=NULL,ternary=F,thresh=NULL,attribute_list){
  ...
  anMHD_mat=distance(ternaryData) 
  anMHD_melted=melt_dist(anMHD_mat)
  
  
  distance_table=anMHD_melted
  
  
  names(distance_table)[3:(dim(distance_table)[2])]=c("anMHD")
