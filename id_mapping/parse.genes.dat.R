...
#Process the file
con = file("ECK_name_mapping/genes.dat", "r")
...
while ( TRUE ) {
    ...
    if(identical(NoOfAtt,integer(0))==FALSE){
      genes.dat[[i]][[att.found]]=sub(att.found,"",line)  ###What if there are multiple lines for like TYPES-A, TYPES-B, TYPES-C...
      temp.att.found=att.found ###temporalily store the att.found in case the next linn is a continuation of the same attribute
      next  
    }
...
save(genes.dat,file="ECK_name_mapping/genes.dat.RData")
