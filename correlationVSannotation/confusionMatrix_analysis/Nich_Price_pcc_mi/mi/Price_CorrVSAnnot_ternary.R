...
#===============================
load("Data/Price_pairwisePCC_longTable_coannotations.RData")

load("Data/price_mi_ternary_allAnnot.RData")
temp=dplyr::left_join(Price_pairwisePCC_longTable_coannotations[,c("strain1","strain2")],
                      price_mi_ternary_allAnnot[,c("strain1","strain2","mi_ternary")],
                      by=c("strain1","strain2"))
##anyNA(temp$mi_ternary) #FALSE. It seems all pair of strains are found. Should be good to proceed
mi_ternary=temp$mi_ternary
Price_pairwisePCC_longTable_coannotations[,3]=mi_ternary
names(Price_pairwisePCC_longTable_coannotations)[3]="mi_ternary"
large_df=Price_pairwisePCC_longTable_coannotations
distance="mi_ternary"
filename_partial="Price"
...
plot_metrics=function(samples,alpha,size,four_annot_list,metric,x_lab="high similarity -- ranked pairs -- low similarity",y,legend_title="Annotation set(s)"){
  ...
  ggplot()+theme_minimal()+theme(legend.title=element_text(size=text_size),
                                 ...
                                 axis.title=element_text(text_size))+ 
