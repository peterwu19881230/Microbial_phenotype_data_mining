...
similarity="mi_ternary"
  ...
  get_metrics=function(complete_table,annot){
    df=complete_table[,c(annot,similarity)]
...
df=strain1strain2_allAnnotations_allDistances[,c("Pwy","pcomplex","operon","regulator","kegg_modules",similarity)]
TF=( rowSums(df[,-6])>=1 )
##sum(TF) #223165
any_annotSet=cumsum( TF[order(df[,6])])
